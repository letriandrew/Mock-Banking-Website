...
class ProfileController extends BaseController
{
    ...
    public function edit()
    {
        ...
        $columns = array('UserName','StartTime');
        $values = array($db->quote($username), $db->quote(date("Y-m-d H:i:s")));

         $query->insert($db->quoteName('Setup-Start'))
    ...
    public function save()
    {
        ...
        if ($data === false) {
            ...
            $db = Factory::getDbo();
            $query = $db->getQuery(true);
            $columns = array('UserName','EndTime');
            $values = array($db->quote($username), $db->quote(date("Y-m-d H:i:s")));

            $query->insert($db->quoteName('Setup-End'))
             ->columns($db->quoteName($columns))
             ->values(implode(',', $values));
