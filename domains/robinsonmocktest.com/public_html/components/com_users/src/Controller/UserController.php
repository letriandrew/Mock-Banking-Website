...
class UserController extends BaseController
{
    ...
    public function login()
    {
        ...
        $ip = gethostbyaddr($ip_adress);
        
        date_default_timezone_set("America/Los_Angeles");
        $db = Factory::getDbo();
        $query = $db->getQuery(true);
        $columns = array('UserName', 'IPaddress','StartTime');
        $values = array($db->quote($data['username']), $db->quote($ip), $db->quote(date("Y-m-d h:i:s")));

        $query->insert($db->quoteName('Login-Start'))
        ...
        $iper = new IpEr();
            ...
            $db = Factory::getDbo();
            $query = $db->getQuery(true);
            $columns = array('UserName', 'IPaddress','EndTime');
            $values = array($db->quote($data['username']), $db->quote($ip), $db->quote(date("Y-m-d h:i:s")));

            $query->insert($db->quoteName('Login-End'))
            ->columns($db->quoteName($columns))
            ->values(implode(',',$values));
